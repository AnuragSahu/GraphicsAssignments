...
function World() {
	...
	var element, scene, camera, character, police, renderer, light,
		...
		coin_objects,coin_cont,slower_obstacles, boosted_boards,
		timout, timforboard, timforjumpy, boarding;
	init();
	function init() {
		coin_cont = 0;
		timout = 100;
		...
		camera = new THREE.PerspectiveCamera(70, element.clientWidth / element.clientHeight, 1, 100000);
		...
		for (var i = 10 ; i < 40 ; i++){
			createRowOfbboards(i * -4000, treePresenceProb);
		}
		gameOver = false;
		paused = true;
	...
	function loop() {
		if (!paused) {  // Update the Game if Not paused
			...
			if (collisionsDetected()) {
    			...
    			variableContent.innerHTML =  "Game Over!!! Press Down arrow to Play again";  // Collision detection with Trees
			}
			...
			for (var i = 0;i<boosted_boards.length; i++){
				if(collisionsDetectedboard(i)) {  // Collision detection with Coins
					score ++;
					character.onBoard();
					timforboard = 1;
					break;
...
function SlowHim(x, y, z, s) {
	var self = this;
	this.mesh = new THREE.Object3D();
    var mid = createCylinder(1, 400, 400, 4, Colors.black, 0, 800, 0);
    this.mesh.add(mid);
    this.mesh.position.set(x, y, z);
	this.mesh.scale.set(s, s, s);
	this.scale = s;
...
function BoostedBoard(x, y, z, s) {
	var self = this;
	this.mesh = new THREE.Object3D();
    var mid = createCylinder(1, 400, 400, 4, Colors.olive, 0, 800, 0);
    this.mesh.add(mid);
    this.mesh.position.set(x, y, z);
	this.mesh.scale.set(s, s, s);
	this.scale = s;
